...
public  class TxtFileParser extends FileParser {
	...
	public Iterator<String> readBlock(FileSettingsToSend fileSetting) {
		...
		try {
			return new Iterator<String>() {
				List<String> lines = Files.lines(Paths.get(fileSetting.getFile().getPath())).collect(Collectors.toList());
				
				Iterator<String> iterator = lines.iterator();
				String currentLine = null;
				Map<Integer,String> attributeIndex = extractAttributeIndexes(iterator.hasNext()?iterator.next():null, listAttribute, lineAttributeDelimitor);
				...
				public String next() {
					...
					if(iterator.hasNext()) System.out.println("there is data "+attributeIndex.size());
	...
	Map extractAttributeIndexes(String firstLineFile, List<String> listAttribute, String delimitor) {
		...
		if(firstLineFile == null) return null;
	 ...
	 * @param delimitor
	 * @return extract all value of requested attribute on that line, with a form key1=value1, key2=value2
	 * 
	 */
