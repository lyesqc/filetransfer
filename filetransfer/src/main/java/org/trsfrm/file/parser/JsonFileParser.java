...
public class JsonFileParser extends FileParser {
	...
	public void getListNodeObject(JsonNode node, String[] delimitor, List<JsonNode> listNode) {
		...
		/**
		 * of current delimitor not equal to current node then flush listNode 
		 * and exit, meaning delivered access path not convinient to file format
		 */
		/*if(node.get(delimitor[0]) == null) {
			listNode = null;
			return;
		}*/
		if (node.isObject()) {
